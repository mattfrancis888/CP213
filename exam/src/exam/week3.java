...
//	       khan
		  ...
		  //But java now does this auotmatically, eg:
		  //Integer integerObject = 42; Uboxing
		  //int i = integerObject;; Boxing
		  
		  //•A computer has two forms of memory
		  //Secondary memory is used to hold files for PERMANENT STORAGE
		  //Main memory is used by a computer when it is RUNNING A PROGRAM (eg; variables)
		  
		  //The == operator only checks that two class type variables have the same memory addres
		  //Unlike the equals method, it does not check that their instance variables have the same valu
		  
		  //A copy constructor is a constructor with a single argument of the same type as the clas
		  //public class Employee {
//		    private int id;
//		    private String name;
//		  
//		    public Employee(Employee employee) {
		  //do not do  (shallow copy)
		  //name = employee.name //dangerous
		  //do (deep copy; its a deep copy because it creates a new instance) (this also applies to situations such as privacy leaks)
		  //name = New String(employee.name)
		  
		  // class that contains no methods (other than constructors) 
		  //that change any of the data in an object of the class is called an immutable clas
//		    }
